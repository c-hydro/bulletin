...
script_folder=$system_library_folder'fp_africa/'
...
script_download_folder='/home/fp/library/hyde/bin/downloader/gfs'
script_download_file=$script_download_folder'/hyde_downloader_nwp_gfs_nomads.py'
setting_file_download=$fp_folder'/fp_tools_preprocessing/nwp/gfs025/hyde_downloader_nwp_gfs_nomads_realtime_africa.json'
...
if [ -f $path_file_lock_def_start ] && [ -f $path_file_lock_def_end ]; then   
    ...
    python3 $script_download_file -settings_file $setting_file_download -time "$time_now" || { rm $path_file_lock_def_start ; echo " ===> EXECUTION ... FAILED" ; exit 1; } 
